...
@section('content_resepobat')
  ...
  <form action="/resepobat/resepobat/simpan" method="post">
    {{ csrf_field() }}
    <label for="ID_REKAM_MEDIS">ID_REKAM_MEDIS : </label><br>
                  <select name="ID_REKAM_MEDIS" id="ID_REKAM_MEDIS" class="form-control">
                    <option>ID_REKAM_MEDIS </option>
                    @foreach($rekammedis as $rm)
                    <option value ="{{ $rm->ID_REKAM_MEDIS}}"> {{ $rm->ID_REKAM_MEDIS}} </option>
                    @endforeach
                  </select><br/>
    ...
    JUMLAH : <input type="text" name="JUMLAH" value = "" required="required"> <br/>
